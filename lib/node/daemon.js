console.log(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH)
      ...
      telehashTransport.start('nfd-th', function(self){
        ...
        self.listen("ndn", function(err, packet, chan, callb){
          ...
          for (var i = 0; i < Faces.length; i ++){
            if (Faces[i].hashname == chan.hashname){
              console.log(Faces[i].hashname +  chan.hashname,  Faces[i].transport.listener)
              isoldFace = true
              chan.callback = Faces[i].transport.listener
              Faces[i].transport.channel = chan
              chan.callback(err, packet, chan, callb)
              console.log("rejuvinating old face")
            }
          ...
          if (isoldFace != true){
            ...
            face.transport.connect(face, function(){callb(true);chan.send({js: "packet"}); face.transport.listener(err, packet, chan, callb);console.log("new telehash face connected on server")})
          }
        });
      })
    }
