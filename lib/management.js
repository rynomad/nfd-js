var FIB = require('./ndn-FIB.js')
  ...
  , ndn = require('ndn-lib')
  , th = require("ndn-telehash-transport").transport
...
managment.faces.create = function(requestingFaceID, name) {
  ...
  if (parameters.protocol == "tcp") {
    ...
    var face = new Face({host:1, port: 1, getTransport: function(){return new th(parameters.hashname) }})
    face.transport.connect(face, function(){
      if (parameters.nextHop){
        var na = new ndn.Name('localhost/nfd/fib/add-nexthop')
        parameters.nextHop.faceID = face.id
        var d = new ndn.Data(name.appendSegment(0), new ndn.SignedInfo(), JSON.stringify(parameters.nextHop))

        d.signedInfo.setFinalBlockID(name.components[name.components.length - 1])
        d.signedInfo.setFields()
        d.sign()
        var enc = d.wireEncode()
        na.append(enc.buffer)
...
managment.fib.addNextHop =  function(requestingFaceID, name){
  ...
  function respond() {
    var resp = new ndn.Data(name.appendSegment(0), new ndn.SignedInfo(), JSON.stringify({"success":true}))
    resp.signedInfo.setFinalBlockID(name.components[name.components.length - 1])
