...
ndnx.init = function(opts, callback){
  ...
  ndnx.face = new ndn.Face({host: 'localhost', port: opts.port || 6464 , onopen: function(){console.log("ndnx face open")}})
  function onData () {
    callback()

    console.log("ndnx face open")
    var closure = new ndn.Face.CallbackClosure(null, null, ndnx.onInterest, param.uri, ndnx.face.transport)
    ndn.Face.registeredPrefixTable.push(new RegisteredPrefix(param.uri, closure))
  }
  ndnx.face.expressInterest(interest, onData)
}
